...
const MenuUserC = ({ isMenuOpen }) => {
  ...
  return (
    ...
    <div className={`sidebar ${isMenuOpen ? 'open' : ''}`}>
      ...
      <h3 className="mx-5 my-4 text-center text-light">Menu</h3>
      <ul className="navbar-nav ms-auto">
          <li className="nav-item mb-3">
            <a className="nav-link"   onClick={() => handleNavigation('/chef-service-dashbord', null)}>
              <i className="bi bi-house-door me-1"></i> Tableau de bord
            </a>
          </li>
          <li className="nav-item mb-3">
            <a className="nav-link " onClick={() => handleNavigation('/mon-profile-chef', { matricule })}>
              <i className="bi bi-person me-1"></i> Mon profile
            </a>
          </li>
          <li className="nav-item mb-3">
            <a className="nav-link" onClick={() => handleNavigation('/create-conge-chef', {matricule})}>
              <i className="bi bi-calendar-plus me-1"></i> Soumettre une nouvelle demande de congé
            </a>
          </li>
          <li className="nav-item">
          <a className="nav-link " href="#" onClick={() => handleNavigation('/conge-list-chef', null)}>
            <i className="bi bi-calendar-plus me-1"></i> Gerer les demandes de congés
          </a>
        </li>

          <a className="nav-link mb-3" onClick={() => handleNavigation('/notifs-chef', { matricule })} >
