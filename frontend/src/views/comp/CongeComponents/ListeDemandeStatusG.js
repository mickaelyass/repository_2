...
const ListeDemandesParStatusG = ({ status  }) => {
  ...
  return (
    <div>
      <h2 className="card-title    py-2 ps-2 ">Liste des demandes de congés {status} par le chef Service</h2>
        ...
        {demandes.map((demande) => (
          ...
          {demande.piecesJointes && (
            <div className="mt-2">
              ...
              <button className="btn btn-primary" disabled onClick={() => handleManageDemande(demande.id_cong)}>
                 Deja Gérer
              </button>
            </div>
          </div>
